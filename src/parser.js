...
function parse_chunk(strChunk, result, data) {
    ...
    while (buf.cursor < buf.str.length) {
        ...
        switch (data.processing) {
            ...
            case '(@1': {
                var firstChar = buf.substr.charAt(0);
                switch (firstChar) {
                    case ')': {
                        buf.advance(1);
                        data.processing = null;
                    } break;
                    case /\s/.test(firstChar): {
                    } break;
                    default: {
                        if (/[\s'"]/.test(firstChar)) {
                            data.processing = '(@!';
                            ...
                            buf.advance(1);
                            data.tagStack.push('@');
                            result[result.length] = firstChar;
                            data.processing = '(@_';
                        }
                    }
                }
